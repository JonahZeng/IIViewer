...
void AboutDlg::initUI()
{
    ...
    log->setMinimumSize(QSize(96, 96));  // NOLINT(cppcoreguidelines-avoid-magic-numbers, readability-magic-numbers)
    const QString title = QString(u8"<h1>ISP intermediate photo viewer</h1>\n");
    const QString builder_version = QString(u8"<p>Build by %1, version: %2.%3.%4, branch: %5</p>\n").arg(GIT_USER).arg(IIViewer_VERSION_MAJOR).arg(IIViewer_VERSION_MINOR).arg(IIViewer_VERSION_PATCH).arg(GIT_BRANCH);
    const QString compiler_version = QString(u8"<p>Compiler %1, version: %2, Qt version: %3.%4.%5</p>\n").arg(CXX_COMPILER_TYPE).arg(CXX_COMPILER_VERSION).arg(QT_VERSION_MAJOR).arg(QT_VERSION_MINOR).arg(QT_VERSION_PATCH);
    const QString commith_ash = QString(u8"<p>Commit: %1</p>\n").arg(GIT_HASH);
    label = new QLabel(title + builder_version + compiler_version + commith_ash + u8"<p>Feedback: send e-mail to <a href=\"mailto:zengyangqiao@126.com\">author</a></p>\n"
                                                                      u8"<p>All right reserved  &#169;2022~2026</p>", this);
    ...
    resize(450, 300); // NOLINT(cppcoreguidelines-avoid-magic-numbers, readability-magic-numbers)
