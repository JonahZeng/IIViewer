...
void IIViewer::onSingleImgModeAction(bool check) // NOLINT(readability-function-cognitive-complexity)
{
...
void IIViewer::onDoubleImgModeAction(bool check) // NOLINT(readability-function-cognitive-complexity)
{
...
void IIViewer::onOpenFileAction()
{
    ...
    auto fileName = QFileDialog::getOpenFileName(this, tr("open images"), settings.workPath,
        "Images files(*.jpg *.JPG *.jpeg *.JPEG *.png *.PNG *.bmp *.BMP *.tif *.TIF *.tiff *.TIFF *.heic *.HEIC);;"
        "Raw files(*.raw *.RAW);;"
        "Pnm files(*.pnm *.PNM);;"
        "Pgm files(*.pgm *.PGM);;"
        "Yuv files(*.yuv *.YUV);;"
...
void IIViewer::reLoadFile(int scrollArea) // NOLINT(readability-function-cognitive-complexity)
{
    ...
    else if (dstFn.endsWith(".dng", Qt::CaseInsensitive))
    {
        loadDngFile(dstFn, scrollArea, true);
    }
    else
    {
...
void IIViewer::loadFile(const QString &fileName, int scrollArea) // NOLINT(readability-function-cognitive-complexity)
{
    ...
    {
        loadDngFile(fileName, scrollArea);
    }
    else
    {
        QMessageBox::warning(this, tr("not support"), tr("this format file not support yet!"), QMessageBox::StandardButton::Ok);
...
void IIViewer::setDngRawImage(QString &imageName, BayerPatternType bay, ByteOrderType order, int bitDepth, bool compact, int width, int height, int leftOrRight, const uint8_t* raw_buffer)
{
    ui.imageLabel.at(leftOrRight)->paintBegin = false;
    ui.imageLabel.at(leftOrRight)->paintEnd = false;
    ui.imageLabel.at(leftOrRight)->imgName = &imageName;
    ui.imageLabel.at(leftOrRight)->setDngRawPixmap(bay, order, bitDepth, compact, width, height, raw_buffer);
    ui.imageLabel.at(leftOrRight)->lastCurve = std::array<QPoint, IMG_PREVIEW_ADJ_CURVE_DOTS>{QPoint(0, 0), QPoint(0, 0), QPoint(0, 0), QPoint(0, 0), QPoint(0, 0), QPoint(0, 0), QPoint(0, 0), QPoint(0, 0)};
}

void IIViewer::setDngPnmImage(QString &imageName, int bitDepth, int width, int height, int leftOrRight, const uint8_t* rgb_buffer)
{
    ui.imageLabel.at(leftOrRight)->paintBegin = false;
    ui.imageLabel.at(leftOrRight)->paintEnd = false;
    ui.imageLabel.at(leftOrRight)->imgName = &imageName;
    ui.imageLabel.at(leftOrRight)->setDngPnmPixmap(bitDepth, width, height, rgb_buffer);
    ui.imageLabel.at(leftOrRight)->lastCurve = std::array<QPoint, IMG_PREVIEW_ADJ_CURVE_DOTS>{QPoint(0, 0), QPoint(0, 0), QPoint(0, 0), QPoint(0, 0), QPoint(0, 0), QPoint(0, 0), QPoint(0, 0), QPoint(0, 0)};
}
